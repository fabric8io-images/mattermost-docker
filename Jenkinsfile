...
dockerTemplate{
   clientsNode {
    ws{
      ...
      if (utils.isCI()){
        def snapshotImageName = "fabric8/mattermost:SNAPSHOT-${env.BRANCH_NAME}-${env.BUILD_NUMBER}"
        container('docker'){
        ...
        stage('notify'){
            ...
            def message = "@${changeAuthor} snapshot Mattermost image is available for testing.  `docker pull ${snapshotImageName}`"
            container('docker'){
        ...
        def imageName = "fabric8/mattermost:${v}"
